...
export async function createGenesisBlock(seedHint: string) {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      throw new Error(errorData.error || errorData.details || "Failed to create genesis block");
    } catch {
      throw new Error(`Failed to create genesis block: ${response.status} - ${errorText}`);
    }
  }
