...
function StuckAlert({ block, nowMs }: { block: Block; nowMs: number }) {
  if (!block.waitingStartedAt || nowMs <= 0) return null;

  const elapsed = nowMs - new Date(block.waitingStartedAt).getTime();
...
export default function DashboardPage() {
  ...
  const { data: blocksData, dataUpdatedAt } = useQuery({
    ...
    <RequireMaster>
      <AdminShell>
        <div className="space-y-6">
          ...
          {/* Stuck block alert */}
          {currentBlock && <StuckAlert block={currentBlock} nowMs={dataUpdatedAt} />}
