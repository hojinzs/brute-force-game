      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-slate-600 text-xs">{formatTime(timestamp)}</span>
          </div>
          <code className="text-slate-50 font-mono text-sm">{inputValue}</code>
        </div>
        <div
          className={`relative px-3 py-1 rounded-md ${getSimilarityBgColor(
            similarity
          )}`}
        >
          <span className={`font-mono text-sm font-medium ${getSimilarityColor(similarity)}`}>
