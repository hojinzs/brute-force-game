...
## 1.시스템 아키텍처 개요
본 프로젝트의 MVP 단계는 프론트엔드는 Vercel, 백엔드는 자체 호스팅 NestJS 인프라를 사용합니다.

- Frontend: Next.js (App Router)
- Backend: NestJS + Prisma (PostgreSQL, Redis)
- AI: ChatGPT 4.1 mini (암호 및 힌트 생성)
- Communication: SSE + Socket.io Gateway (실시간 피드/상태 전파)
...
### 2.3 인증 요구사항
- 이메일 기반 회원가입/로그인 지원 (게스트 로그인 지원).
- 자체 백엔드에서 JWT 발급 및 갱신 처리.
- 회원가입 시 닉네임 필수 입력 (2~20자).

## 3. 백엔드 및 데이터베이스 요구사항 (NestJS)

### 3.1 Database 스키마 (PostgreSQL)
- `profiles` 테이블: 유저 메타데이터 및 자원 관리.
  - `id` (UUID), `nickname`, `cp_count` (int), `last_cp_refill_at` (timestamptz).
- `blocks` 테이블: 해킹 대상 정보.
...
### 3.2 보안 정책
- `blocks.answer_hash`는 API 서버 내부에서만 사용하며, 클라이언트에 노출하지 않음.
- 모든 쓰기 연산은 서버에서 검증 후 수행.

### 3.3 컴퓨팅 파워(CP) 엔진
- 서버 부하 감소를 위해 `get_current_cp` 로직을 서비스 계층에서 수행.
- 로직: `MIN(50, cp_count + FLOOR(EXTRACT(EPOCH FROM (NOW() - last_cp_refill_at)) / 60))`

## 4. Backend Services

### 4.1 check-answer 서비스
...
### 4.2 generate-block 서비스 (Gemini API 연동)
...
- 클라이언트는 SSE 스트림을 구독하여 전 세계 오답 피드와 블록 상태 변경을 즉시 수신.
- 블록 상태(blocks.status)가 pending으로 변경되는 이벤트를 감지하여 승리 알림 오버레이 및 입력창 잠금 처리.

## 6. 보안 및 성능 요구사항
- API Rate Limiting: 동일 유저의 `check-answer` 호출을 초당 2회로 제한.
...
1. 개발 환경: Docker Compose로 PostgreSQL/Redis 로컬 구동.
2. 배포:
  - Frontend: Vercel (GitHub 연동 자동 배포).
  - Backend: 자체 호스팅 인프라 배포.
3. 환경 변수 관리: JWT/Redis/DB/AI 키는 서버 환경 변수 또는 비밀 관리 도구에 저장.
