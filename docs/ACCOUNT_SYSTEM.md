...
1. **최초 방문 시**: 온보딩 진행 (자동 생성 안 함)
2. **계정 생성 시점**: 사용자가 "Play as Guest" 버튼을 클릭할 때 명시적으로 생성
3. **정식 로그인 후**: 로컬스토리지에 `has-logged-in` 플래그 저장
4. **로그아웃 후**: 플래그가 있으면/없으면 관계없이 로그아웃 상태 유지 (인증 요구 블록 표시)
